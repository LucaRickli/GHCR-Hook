...
createServer(async (req, res) => {
  AccessLog.debug(`${req.socket.remoteAddress} => ${req.method} ${req.headers.host}${req.url}`)

  const end = (status: number, msg: string) =>
    res.writeHead(status, { 'Content-Type': 'application/json' }).end(`{"error":"${msg}"}`)
